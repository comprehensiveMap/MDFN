    ...
    def forward(
        ...
        for i in range(sequence_output.size(0)):
            ...
            for j in range(1, last_seps[i] + 1):
                current_context_utter = torch.mean(context_word_level[i, (sep_pos[i][j-1] + 1):(sep_pos[i][j] + 1)], dim = 0, keepdim = True)
                current_sa_utter = torch.mean(sa_word_level[i, (sep_pos[i][j-1] + 1):(sep_pos[i][j] + 1)], dim = 0, keepdim = True)
