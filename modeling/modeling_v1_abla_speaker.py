...
class ElectraForMultipleChoicePlus(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        context_utterance_level = [] # 这里为了处理变长序列，需要用到RNN填充相关函数
        
        for i in range(sequence_output.size(0)):
            context_utterances = [torch.max(context_word_level[i, :(sep_pos[i][0] + 1)], dim = 0, keepdim = True)[0]] # (batch_size, 这里不定长， embedding_size)
            
            for j in range(1, last_seps[i] + 1): # response算上 / response那里不算上
        ...
        final_state = context_final_states
