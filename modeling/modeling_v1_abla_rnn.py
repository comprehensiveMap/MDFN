...
class ElectraForMultipleChoicePlus(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        for i in range(sequence_output.size(0)):
            ...
            context_utterance_level.append(torch.max(torch.cat(context_utterances, dim = 0), dim=0, keepdim = True)[0]) # (batch_size, utterances, hidden_size)
            sa_utterance_level.append(torch.max(torch.cat(sa_utterances, dim = 0), dim=0, keepdim = True)[0])

        context_final_states = torch.cat(context_utterance_level, dim = 0)
        sa_final_states = torch.cat(sa_utterance_level, dim = 0)
