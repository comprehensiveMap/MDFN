...
class CNN(nn.Module):
    def __init__(self, config):
        ...
        self.filter1 = Conv1d(config.hidden_size, config.hidden_size, 2)
        self.filter2 = Conv1d(config.hidden_size, config.hidden_size, 3)
        self.filter3 = Conv1d(config.hidden_size, config.hidden_size, 4)
        self.activation = nn.ReLU()
        self.linear = nn.Linear(3 * config.hidden_size, config.hidden_size)
    ...
    def forward(self, inputs):
        ...
        rep1 = torch.max(self.activation(self.filter1(inputs)), -1)[0]
        rep2 = torch.max(self.activation(self.filter2(inputs)), -1)[0]
        rep3 = torch.max(self.activation(self.filter3(inputs)), -1)[0]

        return self.linear(torch.cat([rep1, rep2, rep3], -1))
...
class ElectraForMultipleChoicePlus(ElectraPreTrainedModel):
    ...
    def forward(
        ...
        for i in range(sequence_output.size(0)):
            context_utterances = [self.cnn1(context_word_level[i, :(sep_pos[i][0] + 1)].unsqueeze(0))]
            sa_utterances = [self.cnn2(sa_word_level[i, :(sep_pos[i][0] + 1)].unsqueeze(0))]
            
            for j in range(1, last_seps[i] + 1):
                current_context_utter = self.cnn1(context_word_level[i, (sep_pos[i][j-1] + 1):(sep_pos[i][j] + 1)].unsqueeze(0))
                current_sa_utter = self.cnn2(sa_word_level[i, (sep_pos[i][j-1] + 1):(sep_pos[i][j] + 1)].unsqueeze(0))
